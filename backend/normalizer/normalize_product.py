...
class ProductNormalizer:
    ...
    def parse_currency_from_price(self, raw_price_str: Optional[str], fallback: Optional[str]) -> Optional[str]:
        if fallback and isinstance(fallback, str):
            ...
            mapped = CURRENCY_SYMBOLS.get(fallback, CURRENCY_SYMBOLS.get(fallback_upper, fallback_upper))
            if len(mapped) <= 4:
                return mapped
    ...
    def map_stock_status(self, raw_stock: Optional[str]) -> str:
        if not raw_stock:
            return "unknown"
        s = raw_stock.lower()
    ...
    def normalize_product(self, raw: dict) -> Tuple[Optional[NormalizedProduct], Dict]:
        """Normalize one raw scraped product dict. Returns (NormalizedProduct | None, diagnostics)."""
